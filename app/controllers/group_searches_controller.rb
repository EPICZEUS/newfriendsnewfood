class GroupSearchesController < ApplicationController
	def index
		@searches = GroupSearch.where(user_id: params[:user_id])
	end

	def create
		@search = GroupSearch.new(group_search_params)

		group = Group.all.find { |e| e.users.include?(@current_user) && e.potential_restaurants.include?(@search.restaurant)}
